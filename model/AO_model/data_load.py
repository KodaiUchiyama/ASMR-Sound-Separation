...
class AVGenerator(keras.utils.Sequence):
    def __init__(self, filename, database_path, X1dim=(298, 257, 2), X2dim=(75, 1, 1792, 1), ydim=(298, 257, 2, 1), batch_size=4, shuffle=True):
        'Initialization'
        self.filename = filename
        self.X1dim = X1dim
        self.X2dim = X2dim
    ...
    def __data_generation(self, filename_temp):
        ...
        # Generate data
        # mix_34-144_00069.npy crm_34-144_00069_34-144.npy 34-144_face_emb.npy
        for i, ID in enumerate(filename_temp):
            info = ID.strip().split(' ')
            X1[i, ] = np.load(self.database_path+'AV_model_database/mix/' + info[0])
            for j in range(1):
                y[i, :, :, :, j] = np.load(self.database_path+'AV_model_database/crm/' + info[j + 1])
                X2[i, :, :, :, j] = np.load('/home/kody.uchiyama/speech_separation-master/data/video/face1022_emb/' + info[j + 2])

        return [X1, X2], y

class AOGenerator(keras.utils.Sequence):
    def __init__(self, filename, database_path, X1dim=(298, 257, 2), ydim=(298, 257, 2, 1), batch_size=4, shuffle=True):
        'Initialization'
        self.filename = filename
        self.X1dim = X1dim
        self.ydim = ydim
    ...
    def __getitem__(self, index):
        ...
        # Generate data
        X1, y = self.__data_generation(filename_temp)
